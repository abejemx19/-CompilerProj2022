...
void Symtable :: dump()
{
	...
	int i,h;
	cout << "Var: SymType, DataType, [low:high], value, realvalue, address" << endl;	
	for (i=0; i<tablesize; i++){
		...
		cout << endl;
		*/
		cout << symtab[h]->getName() << ": ";
		cout << symtab[h]->getSymbolType() << ", ";
		cout << symtab[h]->getDataType() << ", ";
		cout << "["<< symtab[h]->getLowRange()<< ":" << symtab[h]->getHighRange() << "]" << ", ";
		cout << symtab[h]->get_value() << ", ";
		cout << symtab[h]->get_realvalue() << ", ";
		cout << symtab[h]->getAddress() << ", ";
		cout << endl;
	    }
	}
}
